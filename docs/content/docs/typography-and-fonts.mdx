...
Takumi does not use system fonts. All fonts must be explicitly loaded into the `GlobalContext` to be available for rendering.

For `@takumi-rs/core` (napi-rs version), [Geist](https://vercel.com/font) and Geist Mono are embedded by default.
